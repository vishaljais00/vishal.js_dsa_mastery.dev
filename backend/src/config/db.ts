...
const DB_SSL = process.env['DB_SSL'] === 'true' || process.env['NODE_ENV'] === 'production' || !!DATABASE_URL;
...
export async function initializeDatabase() {
  try {
    const connection = DATABASE_URL
      ? await mysql.createConnection({ uri: DATABASE_URL, ssl: DB_SSL ? { rejectUnauthorized: false } : undefined })
    ...
    console.warn(`⚠️ [DB Initialization Notice] ${err.message}. Initializing tables via pool.`);
  }
